...
## Family catalog updates

The existing family generator owns the `ferramenta-family` block in
`README.md.src`. After updating `packages/family/src/family.ts`, run
`pnpm build:package` and `pnpm readme:write`. This refreshes the source block
before mdtheme composes the root README. `pnpm readme:check` checks both stages.
Do not hand-edit that block. The catalog overview is project content here;
this repository uses only the outer Sebastian theme, not a sibling frame.
